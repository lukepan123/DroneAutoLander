...
class MPCController:
    def __init__(self):
        ...
        R = ca.vertcat(
            ca.horzcat(
                c_yaw * c_pit,
                c_yaw * s_rol * s_pit - c_rol * s_yaw,
                s_rol * s_yaw + c_rol * c_yaw * s_pit,
            ),
            ca.horzcat(
                c_pit * s_yaw,
                c_rol * c_yaw + s_rol * s_yaw * s_pit,
                c_rol * s_yaw * s_pit - c_yaw * s_rol,
            ),
        ...
        # Next state
        f_quad = ca.vertcat(vx, vy, vz, ax, ay, az, vroll, vpitch, vyaw)

        self.f = ca.Function(
            "f",
            [self.x, self.u],
            [self.x + self.dt * f_quad],
            {"jit": True, "jit_options": {"compiler": "gcc", "flags": ["-O3"]}},
        )
        ...
        for k in range(self.N):
            ...
            dist_vec = (
                X[0:2, k] - xr[0:2]
            )  # (drone - landing_pad) or vice versa, see below
            dist_unit = dist_vec / (
                ca.norm_2(dist_vec) + 1e-6
            )  # If we want drone to move INFRONT, drone should aim opposite direction of dist
            ...
            cost += (
                ...
                * Q_vxy
                * ca.mtimes(
                    (vel_vec - landing_pad_vel_vec).T, (vel_vec - landing_pad_vel_vec)
                )
            )
            cost += (
                sensitivity * Q_chase * ca.mtimes(vel_vec.T, dist_unit)
            )  # incentivizes moving toward landing_pad

            # Rotational error
            cost += (
                Q_rp * (roll_err * roll_err + pitch_err * pitch_err)
                + Q_yaw * yaw_err * yaw_err
            )
            ...
            xr = xr + self.dt * ca.vertcat(
                xr[4] * ca.cos(xr[3]), xr[4] * ca.sin(xr[3]), 0, xr[5], 0, 0
            )
        ...
        # ---------- Solver ----------
        nlp = {"f": cost, "x": opt_vars, "g": g_concat, "p": P}
    ...
    def compute_control(self, node):
        ...
        sol = self.solver(
            x0=x0, lbx=self.lbx, ubx=self.ubx, lbg=self.lbg, ubg=self.ubg, p=P_val
        )
