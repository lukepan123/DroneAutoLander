...
class Orchestrator(Node):
    ...
    def __init__(self):
        ...
        self._max_runtime = 280.0
        self._boundary_limit = 3000.0
        ...
        self.landing_pad_relative_position = [0, 0, 0]
        self.landing_pad_relative_velocity = [0, 0, 0]
        ...
        # ---- UKF diagnostics (per-state 1-sigma + scalars) ----
        # Initialised from filter at t=0; updated every UKF loop tick.
        ...
        )  # SITL ONLY: confirm this topic name matches your sim
    ...
    def _set_single_message_interval(self, message_id, rate, description):
        ...
        self.get_logger().info(
            f"Setting {description} (ID: {message_id}) to {rate}Hz..."
        )
    ...
    def _ukf_loop(self):
        ...
        if self._UKF_diag["covar_max_eig"] > 10.0:
            self.get_logger().warn(
                f"UKF: max eigenvalue {self._UKF_diag['covar_max_eig']:.3f} — filter may be diverging"
            )
        ...
        try:
            ...
            if stamp_is_new:
                ...
                # self.get_logger().info(f'Measurement age: {self.meas_age:.1f} ms')
                ...
                accepted = self._UKF_filter.update(
                    z=measurement, measurement_timestamp=stamp_sec
                )
    ...
    def _control_loop(self):
        ...
        if self.controller_state == 2100:
            ...
            ) > self._landing_pad_visual_time_SP:
                self.get_logger().info(
        ...
        if self.controller_state == 4000:
            ...
            err_x = abs(self.landing_pad_relative_position[0])
        ...
        _, _, true_yaw = tf_transformations.euler_from_quaternion(
            [
                self.true_odometry.pose.pose.orientation.x,
                self.true_odometry.pose.pose.orientation.y,
                self.true_odometry.pose.pose.orientation.z,
                self.true_odometry.pose.pose.orientation.w,
            ]
        )
        ...
        self._csv_writer.writerow(
            [
                ...
                self.true_odometry.pose.pose.position.x,
                self.true_odometry.pose.pose.position.y,
                self.true_odometry.pose.pose.position.z,
                # Fix velocity because it is given in body frame not world frame...
                self.true_odometry.twist.twist.linear.x * np.cos(true_yaw),
                self.true_odometry.twist.twist.linear.x * np.sin(true_yaw),
                self.true_odometry.twist.twist.linear.z,
                self._ukf_raw_measurement_stamp,
                *self._ukf_raw_measurement,
                ...
                # Fix velocity because it is given in body frame not world frame...
                self.landing_pad_true_odometry.twist.twist.linear.x
                * np.cos(lp_pad_true_yaw),
                self.landing_pad_true_odometry.twist.twist.linear.x
                * np.sin(lp_pad_true_yaw),
    ...
    def _safety_loop(self):
        ...
        if self._tko_complete_time is not None:
            ...
            if elapsed_since_takeoff >= self._max_runtime:
                self.get_logger().warn(
                    f"{self._max_runtime} seconds elapsed since takeoff - Initiating RTL"
                )
